corr <- function(directory, threshold = 0) {
  ...
  completethreshold <- subset(completetotal, nobs > threshold)
 
  files <- list.files(directory, full.names=TRUE) # creates a list of files
  mydata <- data.frame()
  correlations <- vecstor("numeric")
  for (i in completethreshold[, 1]) {
  # loops through the files, rbinding them together
    mydata <- read.csv(files[i])
    keep <- complete.cases(mydata)
    subset <- mydata[keep,]
    correlations <- c(correlations, cor(subset$sulfate, subset$nitrate))
  }
  correlations
}